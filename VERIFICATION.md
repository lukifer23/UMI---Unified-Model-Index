# UMI v0.3.6 verification record
...
| Check | Outcome |
|---|---|
| `uv sync --frozen --extra dev --no-editable --reinstall-package unified-model-index` | passed; installed UMI 0.3.6 as a wheel from the committed lock |
...
| `uv run pytest --cov=umi --cov=analysis --cov=scripts --cov-report=term-missing --cov-fail-under=90` | 108 passed; 92.30% combined coverage |
...
| `umi bundle validate --data-dir data/pilots/v0.3/raw` | passed; acceptance manifest admits 33 records and excludes 8 diagnostic records with 0 unready scored records |
| all twelve offline `umi ingest --source ...` commands | passed: AA composite facts, AA HLE, CursorBench, Epoch ECI, Epoch benchmarks, both Arena cohorts, DeepSWE, and four lab-release sources |
...
| `umi certificate` for Opus/Kimi/GLM | passed; six common stable panels, 18 selected records, four exact artifact checksums, 64 joint scenarios, deterministic result fingerprint `a205c5b9…4592a1d` |
| portable dashboard packaging | passed at 1440 px and 390 px; six charts rendered, source dialog passed, no overflow, external-request, or browser-error failure |
...
- The configured capability matrix contains 75 model/benchmark cells: 28 ready scored, 3 diagnostic
  measurements, 1 diagnostic reference, 2 vendor-claim-only, and 41 missing.
- Every model-specific score is labeled `real evidence — model-specific partial estimate`; it is not
  a UMI rank. Opus, Sol, and Kimi use seven of 15 Capability families across three domains with
  63.125% coverage; GLM uses six families across three domains at 49.375%; Fable remains on one
  family at 8.25%. Opus, Sol, and Kimi clear the Capability-only coverage and breadth gates, but not
...
- exact-configuration, common-cohort capability results for the 41 missing cells, beginning with

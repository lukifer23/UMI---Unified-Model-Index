...
def score_capability(dataset: Dataset, config: ProjectConfig) -> ComponentComputation:
    ...
    for item in dataset.benchmarks:
        grouped[(item.benchmark_id, item.model_id)].append(item)
    ...
    for benchmark_id in sorted({key[0] for key in grouped}):
        definition = definitions[benchmark_id]
        raw: dict[str, float] = {}
        for (candidate_id, model_id), records in grouped.items():
        ...
        benchmark_scores[benchmark_id] = normalized.scores
        benchmark_provisional[benchmark_id] = normalized.provisional
    ...
    for model_id in model_ids:
        ...
        for domain in Domain:
            ...
            for members in family_members.values():
                ...
                weights = {member: definitions[member].weight for member in members}
                family_score, _ = weighted_available(member_values, weights)
                if family_score is not None:
                    family_values.append(family_score)
                    family_weights.append(max(definitions[member].domain_cap for member in members))
